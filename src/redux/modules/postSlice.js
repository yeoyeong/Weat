...
export const loadRoomDetailDB = (id) => {
  return async function (dispatch) {
    try {
      ...
      const response_detail_users = await instance.get(
        `/api/rooms/${id}/users`
      );
      const response_detail_storeList = await instance.get(
        `/api/rooms/${id}/storeList`
      );
  ...
  return async function (dispatch) {
    try {
      const res = await instance.delete(
        `api/rooms/${roomId}/removal/${storeId}`
      );
      const detail_storeList = await instance.get(
        `/api/rooms/${roomId}/storeList`
      );
