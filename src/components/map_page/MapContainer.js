...
const Keyword = ({ detail_data, myLocation }) => {
  ...
  useEffect(() => {
    ...
    searchStore.map((data) => {
      ...
      bounds.extend(
        new kakao.maps.LatLng(data.position.lat, data.position.lng)
      );
    });
  ...
  const FirstSearchModal = (props) => {
    ...
    return (
      <SpeechBubble
        ...
        ref={modalRef}
        onClick={() => FirstUpload(firstSearchStore[0])}
      >
        <li>
  ...
  const SearchModal = (props) => {
    ...
    return (
      <SpeechBubble
        speech_bubble={speech_bubble}
        ref={modalRef}
        onClick={() => navigate(`/review/${searchMarkersInfo.storeId}`)}
      >
        <li>
        ...
        <DiscoveryUserInfo>
          <CharacterfaceWrap eyes={userEye(searchStore[0].eyes)}>
            <NewCharacterface fill={searchStore[0].faceColor} />
          </CharacterfaceWrap>
  ...
  const RoomStoreModal = (props) => {
    ...
    return (
      ...
      >
        ...
        <DiscoveryUserInfo>
          <CharacterfaceWrap eyes={userEye(info.userInfo.eyes)}>
            <NewCharacterface fill={info.userInfo.faceColor} />
          </CharacterfaceWrap>
  ...
  const MyLocationStoreModal = (props) => {
    ...
    return (
      ...
      >
        ...
        <DiscoveryUserInfo>
          <CharacterfaceWrap eyes={userEye(myLocationInfo.userInfo.eyes)}>
            <NewCharacterface fill={myLocationInfo.userInfo.faceColor} />
          </CharacterfaceWrap>
  ...
  const TagFilterStoreModal = (props) => {
    ...
    return (
      ...
      >
        ...
        <DiscoveryUserInfo>
          <CharacterfaceWrap eyes={userEye(filterInfo.userInfo.eyes)}>
            <NewCharacterface fill={filterInfo.userInfo.faceColor} />
          </CharacterfaceWrap>
  ...
  const SearchUserEye = () => {
    return eyeList.filter(
      (row) => row.includes(searchMarkers[0]?.userData?.eyes) && row
    );
  };
    ...
    <>
      ...
      >
        ...
        {markers.map((marker, i) => (
          <div key={marker.storeId}>
            <CustomOverlayMap position={marker.position}>
              <SearchLocationicon
                eye={userEye(marker.userInfo.eyes)}
                onClick={() => {
                  setInfo(marker);
                }}
              >
        ...
        {searchMarkers.map((marker) => (
          ...
          >
            ...
            {searchMarkersInfo &&
              searchMarkersInfo.content === marker.content && (
                <SearchModal
                  onClose={() => {
            ...
            {myLocationInfo && myLocationInfo.storeId === marker.storeId && (
              <MyLocationStoreModal
                onClose={() => {
                  ...
                  setMyLocationInfo();
                  setMyLocationMarkers({
                    position: [...myLocationMarkers.position],
                  });
                }}
              />
            )}
