...
const PostList = ({ roomList, onClose, longPressBackspaceCallback }) => {
  ...
  const [items, setItems] = React.useState(roomList);
  ...
  useEffect(() => {
    ...
    const roomId = items.map((a) => a.roomId);
    const contents_obj = {
      roomSeq: roomId,
    };
      ...
      <SortableItem key={item.roomId}>
        <Inner>
          <PostItem itemAnimationRD={toggle}>
            ...
            <Deletebtn onClick={() => modalOn(item.roomId, item.status)}>
              X
            </Deletebtn>
            <PostItemInner itemAnimationRD={toggle} color={item.status}>
              <li>
                <IconImg>
                  {item.status === "private" && <Private fill="#FFBB55" />}
                </IconImg>
                <IconImg>
                  {item.status === "publicOwner" && <Flag fill="#FF7337" />}
                </IconImg>
                <IconImg>
                  {item.status === "publicGuest" && <Guest fill="#23C7C7" />}
                </IconImg>
              </li>
              <li>
                <h3>{item.roomName}</h3>
              ...
              <li>
                <p>
                  <span>{item.memberNum}</span>members
                </p>
              </li>
            </PostItemInner>
          </PostItem>
  ...
  return (
    <div ref={dragRef}>
      ...
      >
        <Modal
          content="정말 맛방을 나가시겠어요? 그동안 모아뒀던 맛집들이 사라져요;("
          modal={modal}
          setModal={setModal}
          okBtn={() => exitBtn(id, status)}
        />
              ...
              <PostItem>
                <PostItemInner itemAnimationRD={toggle} color={item.status}>
                  <li>
                    <IconImg>
                      {item.status === "private" && (
                        <Private fill={privateColor} />
                      )}
                    </IconImg>
                    <IconImg>
                      {item.status === "publicOwner" && (
                        <Flag fill={ownerColor} />
                      )}
                    </IconImg>
                    <IconImg>
                      {item.status === "publicGuest" && (
                        <Guest fill={guestColor} />
                      )}
                    </IconImg>
                  </li>
                  <li>
...
const PostItem = styled.div`
  ...
  animation: ${({ itemAnimationRD }) =>
      itemAnimationRD ? ItemRotate : "normal"}
    0.4s 1s infinite linear alternate;
  margin: 0;
`;
